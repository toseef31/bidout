  ...
  <v-container class="pa-sm-10 pa-4 corporate-video">
    <v-row>
      <v-col cols="12" sm="12" class="d-flex align-center justify-space-between">
        ...
        <v-btn color="#0D9648" class="text-capitalize white--text mt-5" width="176px" height="54px"
          @click="uploadDocument" :loading="loading" :disabled="loading">Add Document</v-btn>
    ...
    <v-row>
      <v-col cols="3" sm="2" v-for="(doc, index) in companyData.corporateDocuments" :key="index" class="mr-10">
        <div class="doc-col">
          <a :href="doc.attachment" target="_blank" class="text-decoration-none">
            <v-img v-if="get_url_extension(doc.attachment) == 'pdf'" :src="require('@/assets/images/profile/pdf.png')"
              width="80px" class="mx-auto"></v-img>
            <v-img v-else-if="get_url_extension(doc.attachment) == 'xlsx' || get_url_extension(doc.attachment) == 'xls'"
              :src="require('@/assets/images/profile/excel.png')" width="80px" class="mx-auto"></v-img>
            <v-img v-else :src="require('@/assets/images/profile/other.png')" width="80px" class="mx-auto"></v-img>
            <p v-if="edit != doc._id" class="text-truncate d-block">{{ doc.name }}</p>
          </a>
          
          <v-text-field clearable :rules="nameRules" v-model="nameEdit" v-if="edit == doc._id"></v-text-field>
          <v-btn small min-width="36px" v-show="!edit" min-height="36px"
            @click="editDoc(doc)"><v-icon>mdi-pencil</v-icon></v-btn>
          <v-btn small min-width="36px" v-show="edit" :disabled="nameEdit === '' || nameEdit === null && edit == doc._id" min-height="36px"
            @click="saveDoc(doc)"><v-icon>mdi-content-save</v-icon></v-btn>
          &nbsp;
          <v-btn small min-width="36px" min-height="36px"
            @click="deleteDoc(doc)"><v-icon>mdi-trash-can-outline</v-icon></v-btn>
        </div>
      </v-col>
    </v-row>
  </v-container>
