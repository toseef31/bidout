  ...
  <v-col class="my-7 pa-0 qanda-section fill-height" align="start">
    <div v-if="bidDetail.bidData.qAndAEnabled === 'true'">
      <div class="px-6" v-if="(getUserType === 'buyer' && getQAndA.length)">
          ...
          <v-expansion-panel v-model="getPanel" v-for="(item, index) in getQAndForAnswer" :key="index">
            <v-expansion-panel-header>
              ...
              <template>
                <div class="header d-flex flex-column">
                  ...
                  <span class="sub-title mt-4">By {{(item.questionedUserName)}} ({{
                    item.questionedUserCompany
                  }})</span>
                  <span class="sub-title mt-2">{{ item.askedOn._seconds | moment('MM/DD/YYYY') }} - {{
                    item.askedOn._seconds | moment('hh:mma')
                  }}</span>
                </div>
              </template>
            </v-expansion-panel-header>
            <v-expansion-panel-content>
              ...
              <div class="text-center">
                <v-btn color="#0D9648" height="32"
                  class="text-capitalize white--text font-weight-bold save-button px-12" @click="reply(item.id, index)"
                  large :disabled="getLoading[index].loading">
        ...
        <div v-if="getQAndAAnswered.length && getQAndForAnswer.length" class="title-detail mt-6">Answered questions
        </div>
        ...
        <div v-for="(item, index) in getQAndAAnswered" :key="index">
          ...
          <div class="header d-flex flex-column pt-4">
            ...
            <span class="sub-title mt-1">{{ item.answeredOn._seconds | moment('MM/DD/YYYY') }} - {{
              item.answeredOn._seconds | moment('hh:mma')
            }}</span>
          </div>
      ...
      <div class="text-center q-title-detail " v-if="(getUserType === 'buyer' && getQAndA.length === 0)">There are
        currently not any questions, which have been asked by suppliers. Once suppliers as a question this tab will be
        populated.</div>
        ...
        <div v-if="getQAndAAnswered.length">
          <div v-for="(item, index) in getQAndAAnswered" :key="index">
            <div class="header d-flex flex-column pt-4">
              ...
              <span class="sub-title mt-1">{{ item.askedOn._seconds | moment('MM/DD/YYYY') }} - {{
                item.askedOn._seconds
                  | moment('hh:mma')
              }}</span>
            </div>
            ...
            <div class="header d-flex flex-column pt-4">
              ...
              <span class="sub-title mt-4">By {{
                item.answeredUserName
