...
export default {
  ...
  updateProfile({ commit, dispatch, state }, payload) {
    axios.post(`/user/updateUser/${payload.userid}`, {
      email: payload.email, firstName: payload.firstName, lastName: payload.lastName, phoneNumber: payload.phoneNumber, title: payload.title, timezone: payload.timezone,
    })
      .then((responce) => {
        if (responce.status === 200) {
          axios.get(`/user/getUserData/${payload.email}`)
  ...
  inviteUser({ commit, dispatch, state }, payload) {
    axios.post('/company/addInvitedUser/', {
      firstName: payload.firstName, lastName: payload.lastName, company: payload.company, email: payload.email, parent: payload.parent, role: payload.role, companyId: payload.companyId,
    })
      .then((responce) => {
        if (responce.status === 200) {
